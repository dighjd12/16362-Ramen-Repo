...
classdef trajectoryFollowerVerCubicSpiral < handle
    ...
    methods(Static = true)
        ...
        function feedForward(robot, obj, feedback, delay)%,origin)
            ...
            s_to_w_matrix = [cos(obj.lastPoser(3)), -sin(obj.lastPoser(3)), obj.lastPoser(1);
                             sin(obj.lastPoser(3)),  cos(obj.lastPoser(3)), obj.lastPoser(2);
                             0                    ,  0                    , 1];
            %matrix that convert from ref traj start frame to world frame
            rfs_to_w_matrix = [cos(obj.lastPosef(3)), -sin(obj.lastPosef(3)), obj.lastPosef(1);
            ...
            while time < (tf)
                ...
                result1 = rfs_to_w_matrix * [ref_pose(1) ;ref_pose(2) ;1];
                refPose_to_rfs_matrix = [cos(ref_pose(3)), -sin(ref_pose(3)), ref_pose(1);
                                         sin(ref_pose(3)),  cos(ref_pose(3)), ref_pose(2);
                                          0                    ,  0                    , 1];
                result2 = refPose_to_rfs_matrix * rfs_to_w_matrix;
                obj.xpRefArray(i) = result1(1);
                obj.ypRefArray(i) = result1(2);
                obj.thpRefArray(i) = atan2(-result2(1,2),result2(1,1));
                %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
                result3 = s_to_w_matrix * [obj.realArray(i,1) ;obj.realArray(i,2) ;1];
                realPose_to_rfs_matrix = [cos(obj.realArray(i,3)), -sin(obj.realArray(i,3)), obj.realArray(i,1);
                                         sin(obj.realArray(i,3)),  cos(obj.realArray(i,3)), obj.realArray(i,2);
                                          0                    ,  0                    , 1];      
                result4 = realPose_to_rfs_matrix * s_to_w_matrix;
                obj.xpRealArray(i) = result3(1);
                obj.ypRealArray(i) = result3(2);
                obj.thpRealArray(i) = atan2(-result4(1,2),result4(1,1));
                ...
                errorInRobotFrame     = (realPose_to_rfs_matrix)^-1 * errorInStartFrame;

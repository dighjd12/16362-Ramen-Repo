...
%                     upid = kp*e_pro + ki * e_int + kd * e_der;
            ...
            obj.follower.lastPosef = obj.follower.lastPoser;
...
%         end
function success = pickDropObject(obj,robot,objPose)
            %first,from objPose, we get the aqPose
            success = 0;
            aqDis = 0.6;% the distance we have between aqPose and pickupPose
            ...
            ~isnan(lidarPickPose(1))
            if ~isnan(lidarPickPose(1))
        ...
        function lidarPickPose = getLidarPickPose(obj,robot)
            disp('Starting Laser...');
            robot.startLaser();
            pause(1); % wait for laser data to come in

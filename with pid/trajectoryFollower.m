    ...
    methods(Static = true)
        ...
        function a = feedForward(robot, obj, feedback)
            ...
            while time < (tf)
                ...
                leftEncoder = robot.encoders.LatestMessage.Left; %adds travelled distance in m
                ...
                if(feedback)
                    [v_control, w_control] = obj.controller.velFeedback(obj.controller,xErrorArray,yErrorArray,thErrorArray,dtime);%%************
                end
            ...
            disp(max(xErrorArray/1000));
            disp(max(yErrorArray/1000));
            disp(sum(xErrorArray(~isnan(xErrorArray))/1000));
            disp(sum(yErrorArray(~isnan(yErrorArray))/1000));
            %%*************
            a=1;
        end
    end
